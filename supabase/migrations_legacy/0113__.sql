-- Drop the roasting_sessions table if it exists to ensure a clean setup
DROP TABLE IF EXISTS public.roasting_sessions CASCADE;

-- Create the roasting_sessions table to log all roasting activities
...
-- Enable Row Level Security and create policies for the new table
...
-- Create or replace the function to process a roast session
...
-- Create or replace the stock deduction function for orders
