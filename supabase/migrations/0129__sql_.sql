...
DECLARE
    ...
    v_fixed_costs NUMERIC := 20; -- Fixed cost per order, as per pricing.ts
    v_total_order_cost NUMERIC;
    v_profit NUMERIC;
    v_roasted_yield_factor NUMERIC := 0.82; -- Assuming 18% weight loss from green to roasted
    comp RECORD;
    coffee_type_price_green_kg NUMERIC;
    additive_detail RECORD;
    v_grams_additive_per_250g_coffee NUMERIC := 5.0; -- From deduct_stock_for_order
...
BEGIN
    ...
    IF NEW.blend_code IS NOT NULL AND NEW.weight_grams IS NOT NULL THEN
        FOR comp IN
            ...
            WHERE
                bc.blend_code = NEW.blend_code
        LOOP
            SELECT
                ct.price_green_kg
            INTO
                coffee_type_price_green_kg
            FROM
                public.coffee_types ct
            WHERE
                ct.code = comp.coffee_type_code;

            IF FOUND AND coffee_type_price_green_kg IS NOT NULL THEN
                -- Cost of roasted coffee per kg = green_price_per_kg / roasted_yield_factor
                -- This is the raw cost of the roasted coffee, not its selling price.
                DECLARE
    ...
    IF NEW.additives IS NOT NULL AND array_length(NEW.additives, 1) > 0 THEN
        ...
        FOR additive_detail IN
            SELECT
                a.name_ar,
                a.price_per_250g -- This is assumed to be the COST of 250g of the raw additive
            FROM
                public.additives a
            WHERE
                a.name_ar = ANY(NEW.additives)
        LOOP
            IF additive_detail.price_per_250g IS NOT NULL THEN
                -- Cost per gram of this additive
                DECLARE
                    cost_per_gram_of_additive NUMERIC := additive_detail.price_per_250g / 250.0;
                BEGIN
                    -- Total cost for this additive portion in the order
...
-- Create the trigger
CREATE TRIGGER on_order_created_calculate_cost
BEFORE INSERT ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.calculate_order_cost_and_profit();